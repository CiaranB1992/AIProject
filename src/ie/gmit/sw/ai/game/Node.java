...
public class Node {
	...
	public Node[] children(Node[][] maze){		
		...
		if (row > 0 && maze[row][col].hasDirection(Direction.North)) children.add(maze[row - 1][col]); //Add North
		if (row < maze.length - 1 && maze[row + 1][col].hasDirection(Direction.North)) children.add(maze[row + 1][col]); //Add South
		if (col > 0 && maze[row][col].hasDirection(Direction.West)) children.add(maze[row][col - 1]); //Add West
